    ...
    created() {
        ...
        .then(json => {
            if (json.authenticated) {
                ...
                location.href = 'http://127.0.0.1:8080/account/'
            }
        })
    },
    methods: {
        update(product) {
            fetch('/shopping/api/update_product/', {
                mode: 'same-origin',
                method: 'post',
                body: JSON.stringify({
                    'product': product
...
const add = createApp({
    ...
    methods: {
        ...
        search() {
            this.message = '',
            this.product = {},
            fetch('/shopping/api/search_product/', {
                mode: 'same-origin',
                method: 'post',
                body: JSON.stringify({
        ...
        add() {
            fetch('/shopping/api/add_product/', {
                mode: 'same-origin',
                method: 'post',
                body: JSON.stringify({
                    'store': this.selectedStore.shortName,
...
const inventory = createApp({
    ...
    methods: {
        ...
        getZipcode() {
            fetch('/shopping/api/get_zipcode/', {
                mode: 'same-origin',
                method: 'post',
                body: '',
                headers: {
        ...
        list() {
            fetch('/shopping/api/list_inventory/', {
                mode: 'same-origin',
                method: 'post',
                body: JSON.stringify({
